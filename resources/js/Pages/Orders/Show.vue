...
async function openPaymentModal() {
    ...
    try {
        ...
        // Fallback modal keeps the existing attempt accessible if Snap script cannot load.
    } finally {
    ...
    <PublicLayout>
        <section class="bg-neutral-light py-8">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <Alert v-if="latestPayment?.status === 'pending'" tone="info" class="mb-5">
                    Pembayaran masih pending. Kamu bisa membuka ulang modal Midtrans selama token payment belum kedaluwarsa.
                </Alert>
                <Alert v-if="['failed', 'expired', 'cancelled'].includes(latestPayment?.status)" tone="warning" class="mb-5">
                    Payment attempt terakhir belum berhasil. Buat attempt baru jika pesanan belum dibatalkan dan belum dibayar.
                </Alert>
                    ...
                    <div class="grid gap-5">
                        <section class="rounded-md border border-neutral-border bg-white p-5">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-neutral-muted">Timeline proses dari pembayaran sampai pengiriman selesai.</p>
                                </div>
                                <div class="flex flex-wrap gap-2">
                                    <AppButton v-if="canOpenPayment" :loading="snapLoading" @click="openPaymentModal">
                                        <CreditCard class="h-4 w-4" />
                                        Buka Pembayaran
                                    </AppButton>
                                    <AppButton v-else-if="order.can_create_payment_attempt" :loading="retryPaymentForm.processing" @click="createPaymentAttempt">
                                        <CreditCard class="h-4 w-4" />
                                        Coba Bayar Lagi
                                    </AppButton>
                                </div>
                                ...
                                <article v-for="item in order.items" :key="`${item.product_name}-${item.variant_sku}`" class="rounded-md border border-neutral-border p-4">
                                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                        <div>
                                            ...
                                            <p v-if="item.variant_sku" class="mt-1 text-xs text-neutral-muted">SKU {{ item.variant_sku }}</p>
                                        </div>
                        ...
                        <section class="rounded-md border border-neutral-border bg-white p-5">
                            <div class="flex items-center gap-2">
                                <MapPin class="h-5 w-5 text-info" />
                            ...
                            <div class="mt-4 grid gap-2 text-sm">
                                ...
                                <p v-if="order.shipping_city || order.shipping_district || order.shipping_postal_code" class="text-neutral-muted">
                    ...
                    <aside class="grid h-fit gap-5">
                        ...
                        <section class="rounded-md border border-neutral-border bg-white p-5">
                            ...
                            <div v-if="latestPayment" class="mt-4 grid gap-3 text-sm">
                                <div class="flex justify-between gap-3">
                                    <span class="text-neutral-muted">Attempt</span>
                                    <span class="font-semibold text-neutral-text">#{{ latestPayment.attempt_number }}</span>
                                </div>
                                <div class="flex justify-between gap-3">
                                ...
                                <div class="flex justify-between gap-3">
                                    <span class="text-neutral-muted">Midtrans Order</span>
                                    <span class="break-all text-right font-semibold text-neutral-text">{{ latestPayment.midtrans_order_id }}</span>
                                </div>
                                ...
                                <div v-if="latestPayment.expired_at" class="flex justify-between gap-3">
                                    <span class="text-neutral-muted">Kedaluwarsa</span>
                                    <span class="text-right font-semibold text-neutral-text">{{ formatDate(latestPayment.expired_at) }}</span>
                                </div>
                            </div>
                            <p v-else class="mt-4 text-sm text-neutral-muted">Payment attempt belum tersedia.</p>
                        </section>
                        ...
                        <section class="rounded-md border border-neutral-border bg-white p-5">
                            ...
                            <div class="mt-4 grid gap-3 text-sm">
                                ...
                                <p class="rounded-md bg-neutral-light p-3 text-neutral-muted">{{ order.shipment_summary }}</p>
            ...
            <section class="w-full max-w-lg rounded-md bg-white p-5 shadow-xl" role="dialog" aria-modal="true" aria-labelledby="payment-modal-title">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-neutral-muted">Token pembayaran aktif untuk pesanan {{ order.order_number }}.</p>
                    </div>
                ...
                <div class="mt-4 rounded-md border border-neutral-border bg-neutral-light p-4 text-sm">
                    ...
                    <p class="mt-2 break-all text-neutral-muted">Snap token: {{ latestPayment?.snap_token || 'Belum tersedia' }}</p>
                </div>
